...
- **Production (portal + firmware 1.1.1)**: Managed **InfluxDB Cloud Serverless** (account **AmpX** / org **Energy Gateway** / AWS Frankfurt) via `/api/v3/`
- **Legacy**: InfluxDB 2 at `influxdb2.ampx.app` via `/api/v2/` — still deployed; live portal no longer reads it
- **Avoid**: Self-hosting InfluxDB on the Hetzner WordPress host; renaming the Cloud org to AmpX (account ≠ org; quota is one org)
- **AmpX API v3**: Isolated `/api/v3/` writes Cloud Serverless (`INFLUXDB_CLOUD_*`, **org ID** on write URL); same JSON + `X-AmpX-Api-Key` as v2; `api_version` **3.0**
...
### Portal Meter Data Pattern (August 2026; Cloud reads Aug 22)
- **Queries (local + live)**: InfluxQL `GET /query?db=energy_metrics` (not Flux); `time >= now() - 30d` (`READINGS_RANGE_DAYS = 30`). `AMPX_INFLUXDB_ORG` must be **Energy Gateway** (required by config; not currently on the query URL).
- **Table**: newest `READINGS_DISPLAY_LIMIT` (1000) via InfluxQL `LIMIT`; UI must state both caps
- **Meter Data page**: keep only `[ampx_meter_data]` in WP; do not leave Kadence example HTML (100001 / Test was leftover until 22 Aug 2026)
